    ...
    try {
        ...
        for (const dir of dirs) {
            ...
            if (fs.existsSync(dataPath)) {
                ...
                if (spotifyArtistId) {
                    try {
                        const topTracksRes = await axios.get(`https://api.spotify.com/v1/artists/${spotifyArtistId}/top-tracks?market=US`, {
                            headers: { 'Authorization': `Bearer ${token}` }
                        });
                        const topTracks = topTracksRes.data.tracks.slice(0, 5);
